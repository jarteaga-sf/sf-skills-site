...
```javascript
// brandConfig.js — single file to change for each demo
const DEMO_BRAND = {
    name: 'Customer Name',
    primary: '#1a73e8',        // Pull from customer's website
    primaryHover: '#1557b0',   // Darken primary by ~15%
    primaryContrast: '#ffffff',
    secondary: '#34a853',
    logo: '/resource/customerLogo',
    radius: '0.5rem'           // Rounded = modern, 0 = corporate
};
...
export default class BrandedApp extends LightningElement {
    connectedCallback() {
        ...
        host.style.setProperty('--brand-secondary', BRAND.secondary);
...
1. Visit customer's website
2. Open DevTools → pick element with brand color
3. Copy the hex value for primary brand color
4. Use a tool to generate hover (darken 15%) and contrast (white or black)
5. Update `brandConfig.js` with new values
