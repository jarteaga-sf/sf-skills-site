...
async loadDashboard() {
    ...
    // Assign staggered delays for visual cascade
    this.metrics = data.map((metric, index) => ({
        ...metric,
        cardStyle: `animation-delay: ${index * 80}ms`
    }));
...
```javascript
// After data refresh, mark changed values
handleDataRefresh(newData) {
    ...
    // Clear the update flag after animation completes
    setTimeout(() => {
