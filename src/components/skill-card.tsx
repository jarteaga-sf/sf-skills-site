...
export function SkillCard({ skill }: { skill: Skill }) {
  return (
    <Link href={`/skills/${skill.slug}`} className="block group">
      <div className="border border-border rounded-lg px-4 py-3.5 h-full bg-card/50 hover:bg-card hover:border-sf-cloud/30 transition-all duration-200">
        <div className="flex items-start justify-between gap-2 mb-1.5">
          <h3 className="font-semibold text-foreground group-hover:text-sf-cloud transition-colors text-sm tracking-tight">
            {skill.displayName}
          </h3>
          <Badge
            variant="outline"
            className={`text-[10px] shrink-0 ${CATEGORY_COLORS[skill.category] || ""}`}
          >
            {skill.categoryLabel}
          </Badge>
        </div>
